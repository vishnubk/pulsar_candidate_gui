...
png_path = '/media/vishnu/htru_candidates/vishnu/ozstar_cands/latest_msp_candidates/'
df = pd.read_csv('msp_search_candidates_above_50_percent.csv', index_col=False)
df["Beam"] = df.Beam.map("{:02}".format)
df['Cand Name'] = df['pfd_name'].astype(str) + '.png' 
split = df["pfd_name"].str.split("_", expand = True) 
df['Spin Period Ms'] = split[6].astype(str).str[:-2].astype(float)
...
if not os.path.exists('user_classification_msp_cands_above_50_percent.csv'):
    with open('user_classification_msp_cands_above_50_percent.csv', 'w') as outfile:
...
#df2 = pd.read_csv('user_classification_msp_cands_above_50_percent.csv')
#
...
#
#df = df4
print('Candidates with non existing files: ', len(df.index))
existing_candidates = []
...
def update_user_classification_database(pics_value, pulsar_button, harmonic_button, rfi_button, class_a_button, class_b_button, n_clicks, next_button):
    ...
    with open('user_classification_msp_cands_above_50_percent.csv','a') as f:
