...
png_path = '/media/vishnu/htru_candidates/vishnu/ozstar_cands/interesting_cands_90_percent/'
df = pd.read_csv('fresh_cands_above_90_percent_23_sept.csv', index_col=False)
...
if not os.path.exists('user_classification.csv'):
    with open('user_classification.csv', 'w') as outfile:
...
df2 = pd.read_csv('user_classification.csv')
...
def update_user_classification_database(pics_value, pulsar_button, harmonic_button, rfi_button, class_a_button, class_b_button, n_clicks, next_button):
    ...
    with open('user_classification.csv','a') as f:
